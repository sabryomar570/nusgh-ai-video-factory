...
export function hasValidYouTubeOAuthState(input: { code?: string; state?: string; expectedState?: string }) { const code = input.code ?? ""; const state = input.state ?? ""; const expected = input.expectedState ?? ""; return Boolean(code && state && expected && state.length === expected.length && timingSafeEqual(Buffer.from(state), Buffer.from(expected))); }
...
export async function completeYouTubeOAuth(req: Request, res: Response) {
  ...
  if (!hasValidYouTubeOAuthState({ code, state, expectedState: expected })) return res.status(400).json({ error: "invalid oauth state" });
